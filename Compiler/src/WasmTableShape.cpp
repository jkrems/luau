...
struct WasmShapeVisitor : AstVisitor
{
    struct Hasher
    {
    ...
    DenseHashMap<AstExprTable*, WasmTableShape>& shapes;
    DenseHashMap<AstExprTable*, AstLocal*>& bindings;
    ...
    WasmShapeVisitor(DenseHashMap<AstExprTable*, WasmTableShape>& shapes, DenseHashMap<AstExprTable*, AstLocal*>& bindings)
        : shapes(shapes)
        , bindings(bindings)
        , tables(nullptr)
        ...
        if (AstExprLocal* lv = expr->as<AstExprLocal>())
        {
            if (AstExprTable** table = tables.find(lv->local))
            {
                ...
                if (!fields.contains(field))
                {
                    ...
                    // TODO: Add type somehow..? Maybe..?
                    shapes[*table].names.push_back(index);
                }
            }
        }
    }
    ...
    void assignField(AstExpr* expr, AstExpr* index)
    {
        ...
        {
            WasmTableShape& shape = shapes[*table];
    ...
    bool visit(AstStatLocal* node) override
    {
        ...
        if (node->vars.size == 1 && node->values.size == 1)
        {
            if (AstExprTable* table = getTableHint(node->values.data[0]); table && table->items.size == 0)
            {
                ...
                bindings[table] = node->vars.data[0];
            }
        }
...
void predictWasmTableShapes(DenseHashMap<AstExprTable*, WasmTableShape>& shapes, DenseHashMap<AstExprTable*, AstLocal*>& bindings, AstNode* root)
{
    WasmShapeVisitor visitor{shapes, bindings};
    root->visit(&visitor);
}
